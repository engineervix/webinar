...
def import_db_dump(c, dump_file):
    ...
    c.run(f"docker cp {dump_file} webinar-db-1:/tmp/{dump_filename}", pty=True)
    ...
    c.run(
        f'inv exec "db" "pg_restore --clean --no-acl --if-exists --no-owner --host db --username=django_dev_user -d django_dev_db /tmp/{dump_filename}"',
        pty=True,
    )
    # clean up
